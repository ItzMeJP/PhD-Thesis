...
\noindent 
with $n$ and $m$ defined by ``$sim\_annealing/NBR\_ADJ$'' and ``$sim\_annealing/ERR\_ADJ$'', respectively. The ``$DEF\_K0$'' and ``$DEF\_T0$''  are custom initial values for step and temperature. The ``$sim\_annealing/feedback\_num\_steps$'' allows visual update of the optimisation process and the ``$config\_grasps\_parameters$'' are the same described in Section~\ref{cap4:modular_grasping_architecture:sec:grasping_synthesis:subsec:graspit:subsubsec:manual}.
...
%    \item \textbf{sim$\_$annealing/YC:} annealing constant for neighbour generation schedule;
%    \item \textbf{sim$\_$annealing/HC:} annealing constant for error acceptance schedule;
%    \item \textbf{sim$\_$annealing/YDIMS:} number of dimensions for neighbour generation schedule;
%    \item \textbf{sim$\_$annealing/HDIMS:} number of dimensions for error acceptance schedule;
%    \item \textbf{sim$\_$annealing/NBR$\_$ADJ:} adjust factor for neighbour generation schedule
...
Typically, but not mandatory, plugins are client implementations since the localisation server could be loaded a part. Therefore the pipeline need to bring up these outer systems by running a user defined script (or a simple command), called executor. At end, a shutdown scripts (or command), called terminator, is expected to turn off these systems. These commands or files are defined by setting the 
``$executor$\_$cmd$'' and ``$terminator$\_$cmd$'' parameters.

\begin{figure}[h!]
\resizebox{1\textwidth}{!}{%
\begin{tcolorbox}
\centerline{\includegraphics[trim={1cm 0cm 5.5cm 0cm},clip,width=1\linewidth,angle=0]{Cap4/Figuras/mimic_grasping_api_flowchart.pdf}}
\end{tcolorbox}
...
Since several configurations could be done, a profile system is created allowing the load and saving of different configuration setups.
...
\end{tcolorbox}
\caption{The proposed use case hardware setup. The Phoxi 3D Camera S is used to locate the object pose. It is attached on a IRB 2600 robot to set a precise acquisition pose. The 6D Mimic system is responsible to detect the handler tool with its stereoscopic cameras.}
...
A handle tool is designed to a human perform a grasping demonstration, Figure~\ref{fig:tool}. The handler is created following the modular concept since, it needs to support different robots' grippers and detection hardware, if needed. Therefore, the human can teach how to grasp with the same gripper used by the robot.% This tool is detect by a tool localisation algorithm which need to be implemented as a plugin into mimic grasping structure. The same is valid to object localisation algorithm.
...
Regarding the proposed handler firmware, it is possible to communicate, configure it and test it using the firmware window~\ref{fig:firmware_screen_gui}. It is an important configuration menu since it needs to be configured according to the gripper attached to the handler.
...
The Loader, also called Feeder, allows to import an already created dataset into ``Post-processor'' pipeline which should follow the proposed convention described in Section~\ref{cap4:modular_grasping_architecture:sec:grasping_dataset}. It also allows to merge several different datasets into one. It is possible to change the Loader to a ``GraspIt!'' feeder methods into ``Post-processor'' pipeline, as can be verified in Appendix~\ref{ap:grasping_pipeline_configs}. The loader YAML descriptor is defined in Snippet~\ref{code:load_datatset_feeder}. %and its parameters is presented below:
...
Some objects are spherical or radial symmetric, thus some grasping candidates could have symmetric representation~\ref{fig:obj_angle_twin_rep}. Therefore, this could be automatically generated by the object angle twin replica post-processor. This characteristic could improve the candidates' search. It builds a candidate twin based on a custom spherical angle (azimuth $\phi$ and/or polar angle $\Theta$) over the object reference frame. The radius corresponds to the candidate position, therefore it is not a parameter. It is important to note that, the object reference frame should be defined as spherical/radial symmetry origin, otherwise this post-processor will not correctly work. 

\begin{figure}[h!]
	\resizebox{.75\textwidth}{!}{%
		\begin{tcolorbox}
			\centerline{\includegraphics[trim={8.5cm 3.75cm 9.5cm 0.7cm},clip,width=1\linewidth,angle=0]{Cap4/Figuras/obj_twin_rep_2.pdf}}
		\end{tcolorbox}
		\caption{The candidate $\{c_0\}$ and its twin $\{c_1\}$ on a spheric object. Both candidate share the same properties and distance to object's origin $\{O\}$ ($r_0 = r_1$). However, the azimuth and polar angles ($\phi$ and  $\Theta$, respectively) are custom defined by the post-processor. These values are transformed to the rotation matrix following Equation~\ref{eq:rotation_obj_twin}.}
		\label{fig:obj_angle_twin_rep}
	}%end resize box
\end{figure}
	...
	\resizebox{1\textwidth}{!}{%
		\begin{tcolorbox}
			...
			\begin{subfigure}[c]{.3\textwidth}
				\centering
				\includegraphics[trim={0cm 0cm 0cm 0cm},clip,width=1\linewidth,angle=0]{Cap4/Figuras/grasping_gray_bg_01.pdf}
				\caption{The object with grasping candidates frames.}
				\label{fig:grasp_estimation_01}
			\end{subfigure}
			\quad
			\begin{subfigure}[c]{.3\textwidth}
				\centering
				\includegraphics[trim={0cm 0cm 0cm 0cm},clip,width=1\linewidth,angle=0]{Cap4/Figuras/grasping_gray_bg_02.pdf}
...
Since loading the dataset into server demands time, the pipeline allow two types of operation mode: the direct estimation and the pre-load support. The first loads the dataset into rosparam server every time a ROS action's goal request is sent. In other hand, in pre-load support the pipeline requests two ROS action's goals. The first one to load the database into server and the second one to run the selection pipeline. This strategy is useful to accelerate the procedure but the user need to be sure that the loaded database correspond to the object in use when run the selection process (Figure~\ref{fig:grasping_selection_flowchart}).
